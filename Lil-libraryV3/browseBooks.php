...
$booksData = $response ? json_decode($response, true) : null;
    ...
    <section class="section">
        <div class="container">
            ...
            if ($totalBooks > 0) {
                ...
                echo '<a class="pagination-previous" ' . ((int)$currentPage === 1 ? 'disabled' : '') . ' href="?page=1&sort=' . urlencode($sortBy) . '&search=' . urlencode($searchQuery) . '">First</a>';
                echo '<a class="pagination-previous" ' . ((int)$currentPage === 1 ? 'disabled' : '') . ' href="?page=' . max(1, (int)$currentPage - 1) . '&sort=' . urlencode($sortBy) . '&search=' . urlencode($searchQuery) . '">Previous</a>';

                // Page number links with a range of 10 pages
                echo '<ul class="pagination-list">';
                $startPage = max(1, (int)$currentPage - 5);
                $endPage = min($totalPages, (int)$currentPage + 5);
                for ($i = $startPage; $i <= $endPage; $i++) {
                    echo '<li><a class="pagination-link ' . ($i === (int)$currentPage ? 'is-current' : '') . '" href="?page=' . $i . '&sort=' . urlencode($sortBy) . '&search=' . urlencode($searchQuery) . '">' . $i . '</a></li>';
                }
                echo '</ul>';

                // Next and Last buttons
                echo '<a class="pagination-next" ' . ((int)$currentPage === $totalPages ? 'disabled' : '') . ' href="?page=' . min($totalPages, (int)$currentPage + 1) . '&sort=' . urlencode($sortBy) . '&search=' . urlencode($searchQuery) . '">Next</a>';
                echo '<a class="pagination-next" ' . ((int)$currentPage === $totalPages ? 'disabled' : '') . ' href="?page=' . $totalPages . '&sort=' . urlencode($sortBy) . '&search=' . urlencode($searchQuery) . '">Last</a>';
