...
<body>
 ...
 $booksToShow = array_slice($userBooksData, $startIndex, $booksPerPage);
    ...
    if ($totalBooks > 0) {
        ...
        echo '<button ' . ((float)$currentPage === 1 ? 'disabled' : '') . ' onclick="window.location.href=\'?page=' . ($currentPage - 1) . '&sort=' . $sortBy . '&user_search=' . urlencode($searchQuery) . '\'">Previous</button>';
        echo '<span class="page-info">Page ' . $currentPage . ' of ' . $totalPages . '</span>';
        echo '<button ' . ((float)$currentPage === $totalPages ? 'disabled' : '') . ' onclick="window.location.href=\'?page=' . ($currentPage + 1) . '&sort=' . $sortBy . '&user_search=' . urlencode($searchQuery) . '\'">Next</button>';
        echo '</div>';
